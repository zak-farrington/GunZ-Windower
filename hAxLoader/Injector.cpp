...
BOOL InjectDLL(DWORD dwPID, char* strHookDLL)
{
	...
	if(dwAttr == 0xFFFFFFFF)
	{
		...
		switch(dwError)
		{
			...
			case ERROR_PATH_NOT_FOUND:
				MessageBox(GetActiveWindow(), "Error: Invalid path to Hook DLL.", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
				return FALSE;
			break;
	...
	if(dwPID == NULL)
	{
		MessageBox(GetActiveWindow(), "Error: Could not retrieve a valid process ID.", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
		return FALSE;
	}
	...
	{
		MessageBox(GetActiveWindow(), "Error: Could not OpenProcess(PROCESS_ALL_ACCESS, FALSE, ...);", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
		return FALSE;
	}
	...
	if(hKernel == NULL)
	{
		MessageBox(GetActiveWindow(), "Error: Could not LoadLibrary(\"kernel32.dll\");", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
	...
	{
		MessageBox(GetActiveWindow(), "Error: Could not WriteProcessBytes()", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
		CloseHandle(hProcess);
	...
	{
		MessageBox(GetActiveWindow(), "Error: Could not CreateRemoteThread()", "[:: hAx Windower ::]", MB_OK + MB_ICONERROR);
